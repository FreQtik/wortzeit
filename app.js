...
const OFFLINE_CACHE = 'wortzeit-v0.8.12';
const OFFLINE_SHELL = ['./','./index.html','./app.html','./behandler.html','./patient.html','./styles.css?v=0.8.12','./data.js?v=0.8.12','./app.js?v=0.8.12','./manifest-patient.webmanifest','./manifest-therapist.webmanifest'];
...
function gameHeader(title,subtitle,controls='',centerControl=''){return `<div class="game-head"><div class="game-head-left"><div class="game-history-controls"><button class="icon-btn game-back-btn" aria-label="Zurück" title="Zurück">←</button><button class="icon-btn game-forward-btn" aria-label="Vor" title="Vor">→</button></div><div class="game-title"><h1>${esc(title)}</h1><p>${subtitle}</p></div></div><div class="game-center-control">${centerControl||''}</div><div class="game-controls"><div class="game-action-controls">${controls}</div><div class="game-nav-controls">${gameMaterialButton()}<button class="icon-btn game-help-btn" aria-label="Hilfe" title="Hilfe">?</button><button class="icon-btn game-exit-btn" aria-label="Spiel verlassen" title="Zur Spieleauswahl">×</button></div></div></div>`;}
...
function openListEditor(sourceList,returnReviewScope=null){
  ...
  openModal(`<div class="modal-head"><div><div class="eyebrow">Liste bearbeiten</div><h2>${esc(L.title)}</h2></div><button class="icon-btn" data-close-modal>×</button></div>
    ...
    <div id="leChoiceStoryHelp" ${kind==='choiceStory'?'':'hidden'}>${choiceStoryRuleHtml()}</div>
  ...
  $('#leKind').value=kind;
  const updateChoiceStoryHelp=()=>{$('#leChoiceStoryHelp').hidden=$('#leKind').value!=='choiceStory';};
  $('#leKind').onchange=()=>{updateChoiceStoryHelp();};
  updateChoiceStoryHelp();
  const renderFolders=()=>{
  ...
  $('#leSave').onclick=()=>{
    const nextKind=$('#leKind').value,nextEntrySep=$('#leEntrySep').value,nextSyllSep=$('#leSyllSep').value.trim();
    const nextItems=parseEditorItems({...L,kind:nextKind},$('#leText').value,nextSyllSep,editorMode,nextEntrySep);
    if(nextKind==='choiceStory'){const err=validateChoiceStoryEntries(nextItems);if(err)return toast(err);}
    L.title=$('#leTitle').value.trim()||L.title;L.kind=nextKind;L.entrySeparator=nextEntrySep;L.syllableSeparator=nextSyllSep;L.items=nextItems;
    // A copied legacy story may carry pre-structured steps. Once the user edits it,
    // the visible five-line list syntax becomes the single source of truth.
    if(L.kind==='choiceStory'&&Array.isArray(L.choiceStorySteps))delete L.choiceStorySteps;
    rebuildListPairs(L);setListFolders(L,[...folderSet]);L.gameTags=$$('#leGameTags input:checked').map(x=>x.value);if(!L.gameTags.includes('session'))L.gameTags.unshift('session');if(L.review){L.review.status='checked';L.review.checkedAt=new Date().toISOString();}saveState();session=null;game={};closeModal();renderLists();toast('Liste aktualisiert');if(returnReviewScope)setTimeout(()=>openImportReview(returnReviewScope),30);
  };
...
// ---------- Letters ----------
function makeLetterRound(item){
  const chars=[...item.text.toUpperCase()];
  let bank=shuffle(chars.map((c,i)=>({id:`l${i}_${Math.random().toString(36).slice(2,7)}`,c})));
  if(bank.map(x=>x.c).join('')===chars.join('')&&bank.length>1)bank=shuffle(bank);
  return {item,target:chars,bank,placed:Array(chars.length).fill(null),message:'',materialKey:activeMaterialKey(),helpOpen:false,assisted:false,solutionShown:false,completed:false,scored:false};
}
function snapshotLetterRound(g){
  if(!g)return null;
  return {
    item:g.item,
    target:[...(g.target||[])],
    bank:(g.bank||[]).map(x=>({...x})),
    placed:(g.placed||[]).map(x=>x?{...x}:null),
    message:g.message||'',
    materialKey:g.materialKey,
    helpOpen:!!g.helpOpen,
    assisted:!!g.assisted,
    solutionShown:!!g.solutionShown,
...
function newLetterRound(){
  ...
  if(!items.length){game.letters=null;game.letterHistory=[];game.letterFuture=[];return;}
  const used=game.letterUsed||new Set();
  let choices=items.filter(x=>!used.has(`${x.sourceListId||''}:${x.id}`));
  if(!choices.length){used.clear();choices=items;}
  const item=choices[Math.floor(Math.random()*choices.length)];
  used.add(`${item.sourceListId||''}:${item.id}`);
  game.letterUsed=used;
  game.letters=makeLetterRound(item);
}
function advanceLetterRound(){
  const current=snapshotLetterRound(game.letters);
  if(current){
    game.letterHistory=game.letterHistory||[];
    game.letterHistory.push(current);
    if(game.letterHistory.length>80)game.letterHistory.shift();
  }
  game.letterFuture=[];
  newLetterRound();
  renderLetters();
}
function previousLetterRound(){
  const history=game.letterHistory||[];
  if(!history.length)return;
  const current=snapshotLetterRound(game.letters);
  if(current){
    game.letterFuture=game.letterFuture||[];
    game.letterFuture.push(current);
  }
  game.letters=history.pop();
  renderLetters();
}
function forwardLetterRound(){
  const future=game.letterFuture||[];
  if(!future.length)return advanceLetterRound();
  const current=snapshotLetterRound(game.letters);
  if(current){
    game.letterHistory=game.letterHistory||[];
    game.letterHistory.push(current);
  }
  game.letters=future.pop();
  renderLetters();
}
...
function letterEvaluate(g){
  ...
  if(got===target){
    ...
    if(!g.assisted){const current=g;setTimeout(()=>{if(route==='letters'&&game.letters===current&&current.completed)advanceLetterRound();},800);}
    return true;
  }
...
function renderLetters(){
  ...
  const primary=g?`<div class="letter-round-nav"><button class="soft-btn letter-prev-btn" id="letterPrev" ${(game.letterHistory||[]).length?'':'disabled'}>← Vorheriges Wort</button><button class="primary-btn game-check-btn" data-game-primary id="letterNext">Weiter →</button></div>`:'';
  ...
  $('#letterPrev').onclick=previousLetterRound;
  $('#letterNext').onclick=forwardLetterRound;
...
const CHOICE_STORY_SLOT='___';
function parseChoiceStoryTemplate(text){
  const raw=String(text||'').trim();
  const parts=raw.split(CHOICE_STORY_SLOT);
  const slotCount=parts.length-1;
  if(slotCount>1)return {error:'Bitte pro Satz nur einmal ___ verwenden.'};
  if(slotCount===1){
    const prompt=parts[0].trim(),suffix=parts[1].trim();
    if(!prompt&&!suffix)return {error:'Vor oder hinter ___ muss Satztext stehen.'};
    return {prompt,suffix,hasSlot:true};
  }
  if(!raw)return {error:'Der Satz darf nicht leer sein.'};
  return {prompt:raw,suffix:'',hasSlot:false};
}
function validateChoiceStoryEntries(entries){
  const values=(entries||[]).map(x=>String(x?.text??x??'').trim()).filter(Boolean);
  if(!values.length||values.length%5!==0)return 'Eine Auswahlgeschichte braucht immer 5 Zeilen pro Aufgabe: 1 Satz und 4 Antworten.';
  for(let i=0;i<values.length;i+=5){
    const parsed=parseChoiceStoryTemplate(values[i]);
    if(parsed.error)return `Aufgabe ${Math.floor(i/5)+1}: ${parsed.error}`;
  }
  return '';
}
...
function normalizeChoiceStoryStep(step){
  ...
  if((!prompt&&!suffix)||options.length<2||options.length>4)return null;
...
function listToChoiceStory(L){
  ...
  for(let i=0;i<L.items.length;i+=5){
    const parsed=parseChoiceStoryTemplate(L.items[i]?.text||'');
    const options=L.items.slice(i+1,i+5).map(x=>String(x?.text||'').trim()).filter(Boolean);
    if(parsed.error||options.length!==4)return null;
    steps.push({prompt:parsed.prompt,suffix:parsed.suffix,options});
  }
  return {id:`list:${L.id}`,title:L.title,source:'Eigene oder vorbereitete Liste',steps};
}
...
function contextualHelp(){
  const map={
    ...
    letters:['Buchstaben','Baue das gesuchte Wort aus den Buchstaben unten. Du kannst einen Buchstaben antippen oder direkt auf eine beliebige freie Stelle ziehen. Bereits gesetzte Buchstaben lassen sich oben per Drag & Drop tauschen. Mit „Hilfe“ kannst du wahlweise den nächsten Buchstaben einsetzen, alle gleichen Buchstaben zeigen oder die vollständige Lösung anzeigen. „Weiter“ oder die Leertaste nimmt das nächste Wort. Wenn das versehentlich passiert, bringt „← Vorheriges Wort“ das vorherige Wort mit seinem letzten Stand zurück. „Reset“ leert nur die aktuelle Lösung.<br><br><strong>Wofür gedacht:</strong> Wörter bewusst Buchstabe für Buchstabe zusammensetzen – mit genau so viel Unterstützung, wie gerade hilfreich ist.'],
    ...
    choiceStory:['Geschichte bauen','Lies den Satz und wähle eine der vier Möglichkeiten. Die gewählte Antwort erscheint an der vorgesehenen Stelle im Satz. Bei älteren Listen wird sie wie bisher hinten angefügt. Es gibt hier bewusst kein Richtig oder Falsch. Mit „Weiter“ gehst du zum nächsten Satz. Am Ende könnt ihr eure komplette Geschichte noch einmal lesen.<br><br><strong>Wofür gedacht:</strong> Sprache, Entscheidungen, Humor und gemeinsames Erzählen in einer fortlaufenden Situation verbinden.'],
...
// ---------- Service worker ----------
if('serviceWorker' in navigator && location.protocol!=='file:'){navigator.serviceWorker.register('./sw.js?v=0.8.12',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});}
