...
const DEFAULT_STATE = {
  ...
  userLists:[], patients:[], plans:[], storyTitleOverrides:{}, activeListIds:[defaultList.id], recentListIds:[defaultList.id],
...
function currentList(){ return allLists().find(x=>x.id===state.currentListId) || defaultList; }
function rememberList(id){state.recentListIds=[id,...(state.recentListIds||[]).filter(x=>x!==id)].filter(x=>allLists().some(L=>L.id===x)).slice(0,8);}
function activeLists(){const valid=(state.activeListIds||[]).filter(id=>allLists().some(L=>L.id===id));if(!valid.length)return [currentList()];return valid.map(id=>allLists().find(L=>L.id===id)).filter(Boolean);}
function activeMaterialKey(){return activeLists().map(L=>L.id).sort().join('|');}
function activeMaterialTitle(){const ls=activeLists();return ls.length===1?ls[0].title:`${ls.length} Listen gemischt`; }
...
function setCurrentList(id){ if(allLists().some(x=>x.id===id)){ state.currentListId=id; state.activeListIds=[id]; rememberList(id); saveState(); session=null; game={}; updateHeader(); toast(`Liste gewählt: ${currentList().title}`); } }
function toggleActiveList(id){if(!allLists().some(x=>x.id===id))return;const ids=new Set((state.activeListIds||[]).filter(x=>allLists().some(L=>L.id===x)));if(ids.has(id)){if(ids.size===1)return toast('Mindestens eine Liste muss aktiv bleiben.');ids.delete(id);}else{ids.add(id);state.currentListId=id;rememberList(id);}state.activeListIds=[...ids];saveState();session=null;game={};updateHeader();}
function updateHeader(){ $('#currentListTitle').textContent=activeMaterialTitle(); applyI18n(); applyTheme(); }
...
function applyTheme(){ document.body.dataset.theme=state.settings.theme || 'calm'; }
...
function gameHeader(title,subtitle,controls='',centerControl=''){return `<div class="game-head"><div class="game-head-left"><button class="icon-btn game-back-btn" aria-label="Zurück" title="Zurück">←</button><div class="game-title"><h1>${esc(title)}</h1><p>${subtitle}</p></div></div>${centerControl?`<div class="game-center-control">${centerControl}</div>`:''}<div class="game-controls">${controls}<button class="icon-btn game-help-btn" aria-label="Hilfe" title="Hilfe">?</button><button class="icon-btn game-exit-btn" aria-label="Spiel verlassen" title="Zur Spieleauswahl">×</button></div></div>`;}
function bindGameChrome(){ $('.game-back-btn')?.addEventListener('click',gameBack);$('.game-help-btn')?.addEventListener('click',contextualHelp); $('.game-exit-btn')?.addEventListener('click',exitGame); }
...
function planRunBar(){ if(!activePlanRun) return ''; const total=activePlanRun.pkg.steps.length; const step=activePlanRun.index+1; return `<div class="plan-runbar"><div><strong>Therapieplan</strong> <span class="muted">· Schritt ${step} von ${total}</span></div><div class="toolbar"><button class="soft-btn" id="planPrev" ${step<=1?'disabled':''}>←</button><button class="primary-btn" id="planNext">${step>=total?'Plan beenden':'Nächste Übung →'}</button></div></div>`; }
function bindPlanBar(){ if(!activePlanRun) return; $('#planPrev')?.addEventListener('click',()=>{ if(activePlanRun.index>0){activePlanRun.index--; launchPlanStep();}}); $('#planNext')?.addEventListener('click',()=>{ if(activePlanRun.index>=activePlanRun.pkg.steps.length-1){activePlanRun=null; nav('home'); toast('Therapieplan beendet');}else{activePlanRun.index++; launchPlanStep();}}); }
...
function renderHome(){
  $('#view').innerHTML = pageHead('WortZeit','Einfach anfangen.','Aktuelle Liste auswählen, Sitzung starten oder direkt ein Spiel öffnen. Die erweiterten Optionen bleiben im Hintergrund, bis sie gebraucht werden.',`<button class="soft-btn" id="showIntro">Kurze Einführung</button><button class="primary-btn" data-route="session">Sitzung starten</button>`)+`
  <div class="grid cards">
    <button class="action-card" data-route="session"><span class="action-icon">Aa</span><strong>Sitzung</strong><span>Wörter groß anzeigen – antippen zum Weitergehen oder automatisch im Intervall.</span></button>
    <button class="action-card" data-route="games"><span class="action-icon">◇</span><strong>Spiele</strong><span>Wortsalat, Memory, Sortieren, Bildergeschichte, Wortwalze und Silben.</span></button>
    <button class="action-card" data-route="lists"><span class="action-icon">≡</span><strong>Listen</strong><span>Vorbereitete Listen öffnen, eigene Listen anlegen oder alte RTF-Dateien importieren.</span></button>
    <button class="action-card" data-route="patients"><span class="action-icon">◎</span><strong>Patienten</strong><span>Nur Namen und zugewiesene Übungslisten verwalten – lokal auf diesem Gerät.</span></button>
    <button class="action-card" data-route="plans"><span class="action-icon">▤</span><strong>Therapieplan</strong><span>Mehrere Übungen Schritt für Schritt vorbereiten und als Datei weitergeben.</span></button>
    <button class="action-card" id="homePatient"><span class="action-icon">↗</span><strong>Patientenmodus</strong><span>Eine vorbereitete .speechpack-Datei öffnen. Keine Bearbeitungsoptionen sichtbar.</span></button>
  </div>
  <div class="section card"><div class="section-title"><h2>Aktuell</h2><button class="soft-btn" data-route="lists">Liste ändern</button></div><div class="form-row"><div><div class="eyebrow">Liste</div><strong style="font-size:22px">${esc(activeMaterialTitle())}</strong><div class="muted small">${activeItems().length} eindeutige Einträge${activeLists().length>1?` · ${activeLists().map(L=>esc(L.title)).join(' + ')}`:` · ${esc(currentList().category||'')}`}</div></div><div class="spacer"></div><button class="primary-btn" data-route="session">Jetzt verwenden</button></div></div>`;
  $('#homePatient').onclick=()=>enterPatientMode();
  $('#showIntro')?.addEventListener('click',showWelcome);
...
function sessionNext(){
  ...
  if(session.pos>=session.order.length){
    ...
    }else{ session.pos=Math.max(0,session.order.length-step); stopAutoplay(); toast('Ende der Liste'); }
  }
  renderSession();
}
...
function renderLists(){
  ...
  $('#view').innerHTML=pageHead('Material','Listen','Eine Liste allein verwenden oder mehrere Listen für eine Mischübung kombinieren. Importierte Dateien bleiben lokal auf diesem Gerät.',`<button class="primary-btn" id="newList">+ Neue Liste</button><button class="secondary-btn" id="importFile">Datei importieren</button><button class="secondary-btn" id="importFolder">Ordner importieren</button>`)+`
  <div class="active-material card">
    <div><div class="eyebrow">Für die nächsten Übungen</div><strong>${esc(activeMaterialTitle())}</strong><div class="muted small">${activeItems().length} eindeutige Einträge</div></div>
    <div class="active-list-chips">${activeLists().map(L=>`<button class="active-list-chip" data-remove-active="${esc(L.id)}" title="Aus Mischung entfernen">${esc(L.title)} <span>×</span></button>`).join('')}</div>
  </div>
  ...
  <div class="list-browser">
    <div class="card list-tree">
      ...
      <div class="small muted" style="margin-top:8px">Klick auf den Namen = nur diese Liste. Das Häkchen rechts fügt eine Liste zur Mischübung hinzu.</div>
      <div id="listTree" class="section"></div>
    </div>
    <div class="card" id="listDetail"></div>
  </div>
  <input id="filePicker" type="file" accept=".rtf,.txt,.csv,.json" hidden multiple>
  <input id="folderPicker" type="file" webkitdirectory directory multiple hidden>`;
  const renderTree=(q='')=>{
    const norm=q.trim().toLowerCase();
    const filtered=lists.filter(x=>!norm || `${x.title} ${x.category}`.toLowerCase().includes(norm));
  ...
  const renderDetail=()=>{
    const L=currentList();
    const preview=(L.items||[]).slice(0,120);
    $('#listDetail').innerHTML=`<div class="section-title"><div><div class="eyebrow">${esc(L.category||'Liste')}</div><h2>${esc(L.title)}</h2></div><div class="toolbar"><button class="primary-btn" data-route="session">Verwenden</button><button class="soft-btn" id="mixThis">${activeIds.has(L.id)?'✓ In Mischung':'＋ Zur Mischung'}</button><button class="soft-btn" id="recordBank">Aufnahmebank</button><button class="soft-btn" id="exportList">Export</button>${L.bundled?'':`<button class="danger-btn" id="deleteList">Löschen</button>`}</div></div><p class="muted">${L.items?.length||0} Einträge · Typ: ${esc(L.kind||'list')}</p><div class="item-preview">${preview.map((x,i)=>`<div class="preview-row"><span class="muted small">${i+1}.</span> ${esc(x.text)}</div>`).join('')}${(L.items?.length||0)>preview.length?`<div class="preview-row muted">… und ${(L.items.length-preview.length)} weitere</div>`:''}</div><div class="divider"></div><div class="small muted">Quelle: ${esc(L.sourcePath||'lokal angelegt')}</div>`;
    bindRouteButtons();
    $('#mixThis').onclick=()=>{toggleActiveList(L.id);renderLists();};
    $('#recordBank').onclick=()=>openRecordingBank(L);
    $('#exportList').onclick=()=>downloadBlob(`${safeFilename(L.title)}.wortliste.json`,new Blob([JSON.stringify({format:'wortzeit-list',version:1,list:L},null,2)],{type:'application/json'}));
    $('#deleteList')?.addEventListener('click',()=>{if(confirm(`Liste „${L.title}“ wirklich löschen?`)){state.userLists=state.userLists.filter(x=>x.id!==L.id);state.activeListIds=(state.activeListIds||[]).filter(id=>id!==L.id);if(!state.activeListIds.length)state.activeListIds=[defaultList.id];state.currentListId=state.activeListIds[0];saveState();session=null;game={};renderLists();}});
  };
  ...
  $('#importFolder').onclick=()=>$('#folderPicker').click(); $('#folderPicker').onchange=e=>importFiles([...e.target.files],true);
...
async function importFiles(files,folder=false){
  let imported=0; const patientMap=new Map();
  for(const file of files){
    const lower=file.name.toLowerCase(); if(!/\.(rtf|txt|csv|json)$/.test(lower))continue;
    try{
      ...
      const L=makeUserList(file.name.replace(/\.[^.]+$/,''),pieces,category,rel);if(/(^|\/)memory(\/|$)|memo/i.test(rel)){L.kind='pair';L.pairs=[];for(let i=0;i<pieces.length-1;i+=2)L.pairs.push({id:uid('pair'),a:pieces[i],b:pieces[i+1]});}state.userLists.push(L);imported++;
  ...
  saveState();toast(`${imported} Datei${imported===1?'':'en'} importiert`);renderLists();
}
...
function openRecordingBank(L){
  ...
  const show=()=>{
    ...
    $('#bankRec').onclick=async()=>{try{stream=await navigator.mediaDevices.getUserMedia({audio:true});rec=new MediaRecorder(stream);chunks=[];rec.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};rec.onstop=async()=>{const blob=new Blob(chunks,{type:rec.mimeType||'audio/webm'});await audioSet(key,blob);stream?.getTracks().forEach(t=>t.stop());$('#bankStatus').textContent='Gespeichert. Mit „Nächstes“ direkt weiter.';};rec.start();$('#bankStatus').textContent='Aufnahme läuft …';$('#bankRec').disabled=true;$('#bankStop').disabled=false;}catch{$('#bankStatus').textContent='Mikrofon nicht verfügbar. Bitte über START_WINDOWS.bat/localhost oder HTTPS öffnen.';}};
...
async function createMemoryRound(){
  ...
  if(mode==='image'){
    ...
    if(available.length<pairCount){toast('Für diese Kartenzahl fehlen noch Aufnahmen. Es wird normales Memory gestartet.');game.memoryMode='normal';return createMemoryRound();}
    available.slice(0,pairCount).forEach((it,i)=>cards.push({pair:i,type:'text',value:it.text},{pair:i,type:'audio',value:'🔊',audioKey:`${it.sourceListId||state.currentListId}:${it.id}`}));
  }else{
...
function renderMemorySetup(){
  ...
  ].map(([id,t,d])=>`<button class="memory-choice ${mode===id?'selected':''}" data-memory-mode="${id}"><strong>${t}</strong><span>${d}</span></button>`).join('')}</div></div><div><div class="eyebrow">Kartenzahl</div><div class="memory-count-row">${[8,12,16,24].map(n=>`<button class="memory-count-card ${count===n?'selected':''}" data-memory-count="${n}">${n}</button>`).join('')}</div></div><div><div class="eyebrow">Spieler</div><div class="memory-player-row"><button class="memory-choice compact ${players===1?'selected':''}" data-memory-players="1"><strong>1 Spieler</strong></button><button class="memory-choice compact ${players===2?'selected':''}" data-memory-players="2"><strong>2 Spieler</strong></button></div></div><button class="primary-btn memory-start-btn" id="memoryStart">Memory starten</button></div></div></div>`;
  bindPlanBar();bindGameChrome();
  $$('[data-memory-mode]').forEach(b=>b.onclick=()=>{game.memoryMode=b.dataset.memoryMode;renderMemorySetup();});
  $$('[data-memory-count]').forEach(b=>b.onclick=()=>{game.memoryCount=+b.dataset.memoryCount;renderMemorySetup();});
...
function fitMemoryBoard(){
  ...
  const size=Math.floor(Math.min((availableW-gap*(cols-1))/cols,(availableH-gap*(rows-1))/rows,210));
...
function renderMemory(){
  ...
  const g=game.memory;
  const continueBtn=g.pending?`<button class="primary-btn game-check-btn" id="memoryContinue">Weiter</button>`:'';
...
function memoryCardContent(c){if(c.type==='image')return `<img src="${esc(c.value)}" alt="Bildkarte">`;return esc(c.value);}
...
function renderSorting(){
  ...
  const controls=`<div class="score-box">Punkte ${state.stats.sortScore}</div><button class="soft-btn" id="sortReset">Reset</button><button class="soft-btn" id="sortNext">Nächstes</button><button class="primary-btn game-check-btn" id="sortCheck">Prüfen</button>`;
...
function renderStory(){
  ...
  const controls=`<div class="score-box">Punkte ${state.stats.storyScore}</div><button class="soft-btn" id="storyReset">Mischen</button><button class="soft-btn" id="storyNext">Nächstes</button><button class="primary-btn game-check-btn" id="storyCheck">Prüfen</button>`;
  $('#view').innerHTML=`${activePlanRun?planRunBar():''}<div class="game-shell">${gameHeader('Bildergeschichte',`<strong>${esc(title)}</strong> · Bringe die vier Bilder in die richtige Reihenfolge.`,controls)}<div class="game-board story-board"><div class="story-grid">${g.order.map((piece,pos)=>`<button class="story-tile ${g.selected===pos?'selected':''}" draggable="true" data-story-pos="${pos}"><img src="${esc(g.s.pieces[piece])}" alt="Bild ${pos+1}"></button>`).join('')}</div>${g.message?`<div class="game-floating-message ${g.message==='Richtig!'?'success-banner':'error-banner'}">${esc(g.message)}</div>`:''}</div></div>`;
  bindPlanBar();bindGameChrome();requestAnimationFrame(fitStoryBoard);
  $$('[data-story-pos]').forEach(b=>{b.onclick=()=>selectStory(+b.dataset.storyPos);b.ondragstart=e=>e.dataTransfer.setData('text/plain',b.dataset.storyPos);b.ondragover=e=>{e.preventDefault();b.classList.add('drag-over')};b.ondragleave=()=>b.classList.remove('drag-over');b.ondrop=e=>{e.preventDefault();b.classList.remove('drag-over');const a=+e.dataTransfer.getData('text/plain'),c=+b.dataset.storyPos;if(Number.isFinite(a)&&a!==c){[g.order[a],g.order[c]]=[g.order[c],g.order[a]];g.message='';renderStory();}};});
  $('#storyReset').onclick=()=>{g.order=shuffle([0,1,2,3]);if(g.order.join('')==='0123')g.order=[1,0,3,2];g.selected=null;g.message='';renderStory();};
  $('#storyNext').onclick=()=>{newStory(true);renderStory();};
  $('#storyCheck').onclick=()=>{const ok=g.order.join('')==='0123';g.message=ok?'Richtig!':'Noch nicht richtig.';if(ok){state.stats.storyScore++;saveState();renderStory();setTimeout(()=>{newStory(true);renderStory();},650);}else renderStory();};
}
function selectStory(pos){const g=game.story;if(g.selected==null){g.selected=pos;}else if(g.selected===pos){g.selected=null;}else{[g.order[g.selected],g.order[pos]]=[g.order[pos],g.order[g.selected]];g.selected=null;g.message='';}renderStory();}
...
function fitWheelGeometry(){
  ...
  const rowH=clamp(Math.floor(stage.clientHeight/6),58,134);stage.style.setProperty('--wheel-row-h',`${rowH}px`);fitWheelRows();updateWheelTransform();
}
function fitWheelRows(){
  const g=game.wheel;$$('.wheel-row').forEach(el=>{const rowH=el.getBoundingClientRect().height||96;const base=clamp(Math.round(rowH*.68*(g?.fontScale||125)/100),28,116),min=18;let size=base;el.style.fontSize=`${size}px`;while(size>min&&el.scrollWidth>el.clientWidth-44){size-=2;el.style.fontSize=`${size}px`;}});
}
...
function evaluateSyllables(g){
  const built=normalizeWordForSyllables(g.built.join(''));if(!built){g.message='';return;}
  const hit=(g.validWords||[]).find(x=>normalizeWordForSyllables(x.word)===built);g.message=hit?`Wort erkannt: ${hit.word}`:'';
}
function renderSyllables(){
  ...
  $('#syllableReset').onclick=()=>{g.built=[];g.message='';renderSyllables();};$('#syllableNew').onclick=()=>{buildSyllableRound();renderSyllables();};
}
...
function renderChoiceStory(){
  ...
  const center=`<button class="primary-btn choice-next-top" id="choiceNext" ${g.currentChoice==null?'disabled':''}>${g.index>=story.steps.length-1?'Geschichte lesen':'Weiter →'}</button>`;
...
function renderSemantic(){
  ...
  const controls=`<button class="soft-btn" id="semanticReset">Zurücksetzen</button><button class="primary-btn" id="semanticNext">Nächstes Wort</button>`;
  const prompts=[
...
function openPatientEditor(patient=null){
  const p=patient||{id:uid('patient'),name:'',listIds:[],note:''};const lists=[...allLists()].sort((a,b)=>a.title.localeCompare(b.title,'de',{sensitivity:'base',numeric:true}));
...
function openPlanEditor(plan=null){
  const work=structuredClone(plan||{id:uid('plan'),title:'Neuer Therapieplan',steps:[]});
  ...
  openModal(modalHtml());
  function rebind(){
    $('#addPlanStep').onclick=()=>{work.title=$('#planTitle').value;work.steps.push({activity:'session',listId:state.currentListId,listIds:activeLists().map(L=>L.id),options:{}});openModal(modalHtml());rebind();};
    $$('[data-step-activity]').forEach(x=>x.onchange=()=>work.steps[+x.dataset.stepActivity].activity=x.value);$$('[data-step-list]').forEach(x=>x.onchange=()=>{const st=work.steps[+x.dataset.stepList];if(x.value==='__mix__'){st.listIds=activeLists().map(L=>L.id);st.listId=st.listIds[0]||state.currentListId;}else{st.listId=x.value;st.listIds=[x.value];}});
...
function planEditorStep(s,i){const sorted=[...allLists()].sort((a,b)=>a.title.localeCompare(b.title,'de',{sensitivity:'base',numeric:true}));const mixIds=activeLists().map(L=>L.id);const hasMix=mixIds.length>1;const selectedMix=(s.listIds||[]).length>1;const listOptions=(hasMix?`<option value="__mix__" ${selectedMix?'selected':''}>Aktive Mischung · ${mixIds.length} Listen</option>`:'')+sorted.map(L=>`<option value="${esc(L.id)}" ${!selectedMix&&L.id===s.listId?'selected':''}>${esc(L.title)}</option>`).join('');const actOptions=Object.entries(ACTIVITY_LABELS).map(([k,v])=>`<option value="${k}" ${k===s.activity?'selected':''}>${esc(v)}</option>`).join('');return `<div class="plan-step"><div class="plan-index">${i+1}</div><div class="form-row"><div class="field"><label>Übung</label><select data-step-activity="${i}">${actOptions}</select></div><div class="field grow"><label>Material</label><select data-step-list="${i}">${listOptions}</select></div></div><div class="plan-step-actions"><button class="icon-btn" data-step-up="${i}" title="Nach oben">↑</button><button class="icon-btn" data-step-down="${i}" title="Nach unten">↓</button><button class="icon-btn" data-step-remove="${i}" title="Entfernen">×</button></div></div>`;}
function packageFromPlan(plan){
  const listIds=[...new Set(plan.steps.flatMap(s=>(s.listIds?.length?s.listIds:[s.listId]).filter(Boolean)))];const snapshots=listIds.map(id=>allLists().find(L=>L.id===id)).filter(Boolean).map(L=>({id:L.id,title:L.title,category:L.category,kind:L.kind,items:L.items,pairs:L.pairs||[]}));
  return {format:'wortzeit-speechpack',version:1,title:plan.title,createdAt:new Date().toISOString(),steps:plan.steps.map(s=>({...s})),lists:snapshots,settings:{lang:state.settings.lang,theme:state.settings.theme,brightness:state.settings.brightness}};
}
...
function renderPatientMode(){
  const v=$('#view');
  v.innerHTML=`<div class="patient-gate"><div class="patient-panel"><div class="brand-mark" style="margin:0 auto">WZ</div><h1>Übung öffnen</h1><p>Öffne die Datei, die du für deine Übungen bekommen hast. Die Datei wird auf diesem Gerät verarbeitet und in dieser Version nicht hochgeladen.</p><div class="drop-zone" id="patientDrop"><strong style="font-size:20px">.speechpack hier ablegen</strong><p>oder</p><button class="primary-btn" id="patientPick">Datei auswählen</button><input id="patientPackFile" type="file" accept=".speechpack,.json" hidden></div><div class="section"><button class="soft-btn" id="leavePatient">Zur Behandleransicht</button></div></div></div>`;
  $('#patientPick').onclick=()=>$('#patientPackFile').click();$('#patientPackFile').onchange=e=>{const f=e.target.files[0];if(f)loadSpeechpackFile(f,false);};$('#leavePatient').onclick=leavePatientMode;const d=$('#patientDrop');d.ondragover=e=>{e.preventDefault();d.classList.add('drag-over')};d.ondragleave=()=>d.classList.remove('drag-over');d.ondrop=e=>{e.preventDefault();d.classList.remove('drag-over');const f=e.dataTransfer.files[0];if(f)loadSpeechpackFile(f,false);};
}
async function loadSpeechpackFile(file,addToPlans=false){
  try{const text=await file.text();const pkg=JSON.parse(text.replace(/^\uFEFF/,''));if(pkg.format!=='wortzeit-speechpack'||!Array.isArray(pkg.steps))throw new Error('wrong format');
    ...
    importedPatientPackage=pkg;patientMode=true;document.body.classList.add('patient-mode');activePlanRun={pkg,index:0,local:false};launchPlanStep();
...
function contextualHelp(){
  const map={
    ...
    memory:['Memory','Zuerst wählst du Memory-Art, 8/12/16/24 Karten und 1 oder 2 Spieler. Danach startet das Spielfeld. Die Karten drehen sich beim Öffnen. Mit „Paar halten AN“ bleiben zwei geöffnete Karten so lange liegen, bis du oben auf „Weiter“ drückst – praktisch, wenn ihr darüber sprechen möchtet.<br><br><strong>Wofür gedacht:</strong> Begriffe, Bilder oder gehörte Wörter miteinander in Beziehung setzen und wiedererkennen.'],
    ...
    story:['Bildergeschichte','Bringe die vier Bilder in die richtige Reihenfolge: oben links = 1, oben rechts = 2, unten links = 3, unten rechts = 4. Du kannst zwei Bilder antippen, um sie zu tauschen, oder sie ziehen. Wenn du fertig bist, drücke oben rechts auf den grünen Knopf „Prüfen“.<br><br><strong>Wofür gedacht:</strong> Eine Handlung zeitlich ordnen und anschließend in eigenen Worten beschreiben oder erzählen.'],
    ...
    syllables:['Silben','Die sechs Silben liegen auf den sechs Eckpunkten des Hexagons und bleiben dort räumlich stabil. Tippe oder ziehe sie nach oben. Dort kannst du die gewählten Silben auch umsortieren. „Reset“ leert nur deine Auswahl, „Mischen“ lädt neue Silben. Mindestens ein Wort lässt sich in jeder Runde bilden.<br><br><strong>Wofür gedacht:</strong> Silben in einer stabilen räumlichen Anordnung auswählen und schrittweise zu einem Wort zusammensetzen.'],
...
window.addEventListener('keydown',e=>{if(e.key==='Escape'){if($('#modalRoot').innerHTML)closeModal();else if($('#drawer').classList.contains('open'))closeDrawer();else if(GAME_ROUTES.includes(route)){e.preventDefault();gameBack();}}if(route==='session'&&!e.target.matches('input,textarea,select')){if(e.key==='ArrowRight'||e.key===' '){e.preventDefault();sessionNext();}if(e.key==='ArrowLeft'){e.preventDefault();sessionPrev();}}});
